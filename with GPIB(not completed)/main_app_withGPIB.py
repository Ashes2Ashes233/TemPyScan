...
from instrument_controller import Keithley27xxController as InstrumentController
...
class ThermoApp(tk.Tk):
    ...
    def generate_final_report(self):
        ...
        try:
            ...
            start_offset = float(self.report_time_range.get('start', 0) or 0)
            end_offset_str = self.report_time_range.get('end')
            if end_offset_str:
                end_offset = float(end_offset_str)
                report_data['Stop time'] = time.asctime(time.localtime(self.start_timestamp + end_offset))
            else:
                report_data['Stop time'] = "N/A"

            report_data['Start time'] = time.asctime(time.localtime(self.start_timestamp + start_offset))
            ...
            for i, ch_index in enumerate(valid_channels_for_report):
                ...
                try:
                    threshold_val = float(config['threshold']) if config['threshold'] else float('inf')
    ...
    # --- 修改 connect_instrument 方法以接收新参数 ---
    def connect_instrument(self, address, device_type, connection_type):
        self.channel_offset = 0 if device_type == "1-80" else 80
        # --- 将新参数传递给控制器类的构造函数 ---
        self.instrument = InstrumentController(address, connection_type)
    ...
    def _data_acquisition_loop(self, interval):
        while not self.stop_thread.is_set():
            if self.instrument and self.instrument.connected:
                try:
                    read_time = time.time()
                    raw_data = self.instrument.get_data('READ?')  # get_data返回的是列表
                    if raw_data:
                        temps_80ch = np.array(raw_data)  # 直接转换为numpy数组
    ...
    # --- 其余所有辅助方法 (get_channel_configs, get_history, 等) ---
